...
class DeviceDeviceType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is DeviceDeviceType.AKUVOX_LOCK:
            return akuvox_lock()
        if self is DeviceDeviceType.AUGUST_LOCK:
            return august_lock()
        if self is DeviceDeviceType.BRIVO_ACCESS_POINT:
            return brivo_access_point()
        if self is DeviceDeviceType.BUTTERFLYMX_PANEL:
            return butterflymx_panel()
        if self is DeviceDeviceType.DOORKING_LOCK:
            return doorking_lock()
        if self is DeviceDeviceType.GENIE_DOOR:
            return genie_door()
        if self is DeviceDeviceType.IGLOO_LOCK:
            return igloo_lock()
        if self is DeviceDeviceType.LINEAR_LOCK:
            return linear_lock()
        if self is DeviceDeviceType.LOCKLY_LOCK:
            return lockly_lock()
        if self is DeviceDeviceType.KWIKSET_LOCK:
            return kwikset_lock()
        if self is DeviceDeviceType.NUKI_LOCK:
            return nuki_lock()
        if self is DeviceDeviceType.SALTO_LOCK:
            return salto_lock()
        if self is DeviceDeviceType.SCHLAGE_LOCK:
            return schlage_lock()
        if self is DeviceDeviceType.SEAM_RELAY:
            return seam_relay()
        if self is DeviceDeviceType.SMARTTHINGS_LOCK:
            return smartthings_lock()
        if self is DeviceDeviceType.YALE_LOCK:
            return yale_lock()
        if self is DeviceDeviceType.TWO_N_INTERCOM:
            return two_n_intercom()
        if self is DeviceDeviceType.CONTROLBYWEB_DEVICE:
            return controlbyweb_device()
        if self is DeviceDeviceType.TTLOCK_LOCK:
            return ttlock_lock()
        if self is DeviceDeviceType.IGLOOHOME_LOCK:
            return igloohome_lock()
        if self is DeviceDeviceType.HUBITAT_LOCK:
            return hubitat_lock()
        if self is DeviceDeviceType.NOISEAWARE_ACTIVITY_ZONE:
            return noiseaware_activity_zone()
        if self is DeviceDeviceType.MINUT_SENSOR:
            return minut_sensor()
        if self is DeviceDeviceType.ECOBEE_THERMOSTAT:
            return ecobee_thermostat()
        if self is DeviceDeviceType.NEST_THERMOSTAT:
