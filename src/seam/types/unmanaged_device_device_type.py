...
class UnmanagedDeviceDeviceType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is UnmanagedDeviceDeviceType.AKUVOX_LOCK:
            return akuvox_lock()
        if self is UnmanagedDeviceDeviceType.AUGUST_LOCK:
            return august_lock()
        if self is UnmanagedDeviceDeviceType.BRIVO_ACCESS_POINT:
            return brivo_access_point()
        if self is UnmanagedDeviceDeviceType.BUTTERFLYMX_PANEL:
            return butterflymx_panel()
        if self is UnmanagedDeviceDeviceType.DOORKING_LOCK:
            return doorking_lock()
        if self is UnmanagedDeviceDeviceType.GENIE_DOOR:
            return genie_door()
        if self is UnmanagedDeviceDeviceType.IGLOO_LOCK:
            return igloo_lock()
        if self is UnmanagedDeviceDeviceType.LINEAR_LOCK:
            return linear_lock()
        if self is UnmanagedDeviceDeviceType.LOCKLY_LOCK:
            return lockly_lock()
        if self is UnmanagedDeviceDeviceType.KWIKSET_LOCK:
            return kwikset_lock()
        if self is UnmanagedDeviceDeviceType.NUKI_LOCK:
            return nuki_lock()
        if self is UnmanagedDeviceDeviceType.SALTO_LOCK:
            return salto_lock()
        if self is UnmanagedDeviceDeviceType.SCHLAGE_LOCK:
            return schlage_lock()
        if self is UnmanagedDeviceDeviceType.SEAM_RELAY:
            return seam_relay()
        if self is UnmanagedDeviceDeviceType.SMARTTHINGS_LOCK:
            return smartthings_lock()
        if self is UnmanagedDeviceDeviceType.YALE_LOCK:
            return yale_lock()
        if self is UnmanagedDeviceDeviceType.TWO_N_INTERCOM:
            return two_n_intercom()
        if self is UnmanagedDeviceDeviceType.CONTROLBYWEB_DEVICE:
            return controlbyweb_device()
        if self is UnmanagedDeviceDeviceType.TTLOCK_LOCK:
            return ttlock_lock()
        if self is UnmanagedDeviceDeviceType.IGLOOHOME_LOCK:
            return igloohome_lock()
        if self is UnmanagedDeviceDeviceType.HUBITAT_LOCK:
            return hubitat_lock()
        if self is UnmanagedDeviceDeviceType.NOISEAWARE_ACTIVITY_ZONE:
            return noiseaware_activity_zone()
        if self is UnmanagedDeviceDeviceType.MINUT_SENSOR:
            return minut_sensor()
        if self is UnmanagedDeviceDeviceType.ECOBEE_THERMOSTAT:
            return ecobee_thermostat()
        if self is UnmanagedDeviceDeviceType.NEST_THERMOSTAT:
