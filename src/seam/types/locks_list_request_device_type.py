...
class LocksListRequestDeviceType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is LocksListRequestDeviceType.AKUVOX_LOCK:
            return akuvox_lock()
        if self is LocksListRequestDeviceType.AUGUST_LOCK:
            return august_lock()
        if self is LocksListRequestDeviceType.BRIVO_ACCESS_POINT:
            return brivo_access_point()
        if self is LocksListRequestDeviceType.BUTTERFLYMX_PANEL:
            return butterflymx_panel()
        if self is LocksListRequestDeviceType.DOORKING_LOCK:
            return doorking_lock()
        if self is LocksListRequestDeviceType.GENIE_DOOR:
            return genie_door()
        if self is LocksListRequestDeviceType.IGLOO_LOCK:
            return igloo_lock()
        if self is LocksListRequestDeviceType.LINEAR_LOCK:
            return linear_lock()
        if self is LocksListRequestDeviceType.LOCKLY_LOCK:
            return lockly_lock()
        if self is LocksListRequestDeviceType.KWIKSET_LOCK:
            return kwikset_lock()
        if self is LocksListRequestDeviceType.NUKI_LOCK:
            return nuki_lock()
        if self is LocksListRequestDeviceType.SALTO_LOCK:
            return salto_lock()
        if self is LocksListRequestDeviceType.SCHLAGE_LOCK:
            return schlage_lock()
        if self is LocksListRequestDeviceType.SEAM_RELAY:
            return seam_relay()
        if self is LocksListRequestDeviceType.SMARTTHINGS_LOCK:
            return smartthings_lock()
        if self is LocksListRequestDeviceType.YALE_LOCK:
            return yale_lock()
        if self is LocksListRequestDeviceType.TWO_N_INTERCOM:
            return two_n_intercom()
        if self is LocksListRequestDeviceType.CONTROLBYWEB_DEVICE:
            return controlbyweb_device()
        if self is LocksListRequestDeviceType.TTLOCK_LOCK:
            return ttlock_lock()
        if self is LocksListRequestDeviceType.IGLOOHOME_LOCK:
            return igloohome_lock()
        if self is LocksListRequestDeviceType.HUBITAT_LOCK:
            return hubitat_lock()
        if self is LocksListRequestDeviceType.NOISEAWARE_ACTIVITY_ZONE:
            return noiseaware_activity_zone()
        if self is LocksListRequestDeviceType.MINUT_SENSOR:
            return minut_sensor()
        if self is LocksListRequestDeviceType.ECOBEE_THERMOSTAT:
            return ecobee_thermostat()
        if self is LocksListRequestDeviceType.NEST_THERMOSTAT:
