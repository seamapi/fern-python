...
class ThermostatsListRequestDeviceType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is ThermostatsListRequestDeviceType.AKUVOX_LOCK:
            return akuvox_lock()
        if self is ThermostatsListRequestDeviceType.AUGUST_LOCK:
            return august_lock()
        if self is ThermostatsListRequestDeviceType.BRIVO_ACCESS_POINT:
            return brivo_access_point()
        if self is ThermostatsListRequestDeviceType.BUTTERFLYMX_PANEL:
            return butterflymx_panel()
        if self is ThermostatsListRequestDeviceType.DOORKING_LOCK:
            return doorking_lock()
        if self is ThermostatsListRequestDeviceType.GENIE_DOOR:
            return genie_door()
        if self is ThermostatsListRequestDeviceType.IGLOO_LOCK:
            return igloo_lock()
        if self is ThermostatsListRequestDeviceType.LINEAR_LOCK:
            return linear_lock()
        if self is ThermostatsListRequestDeviceType.LOCKLY_LOCK:
            return lockly_lock()
        if self is ThermostatsListRequestDeviceType.KWIKSET_LOCK:
            return kwikset_lock()
        if self is ThermostatsListRequestDeviceType.NUKI_LOCK:
            return nuki_lock()
        if self is ThermostatsListRequestDeviceType.SALTO_LOCK:
            return salto_lock()
        if self is ThermostatsListRequestDeviceType.SCHLAGE_LOCK:
            return schlage_lock()
        if self is ThermostatsListRequestDeviceType.SEAM_RELAY:
            return seam_relay()
        if self is ThermostatsListRequestDeviceType.SMARTTHINGS_LOCK:
            return smartthings_lock()
        if self is ThermostatsListRequestDeviceType.YALE_LOCK:
            return yale_lock()
        if self is ThermostatsListRequestDeviceType.TWO_N_INTERCOM:
            return two_n_intercom()
        if self is ThermostatsListRequestDeviceType.CONTROLBYWEB_DEVICE:
            return controlbyweb_device()
        if self is ThermostatsListRequestDeviceType.TTLOCK_LOCK:
            return ttlock_lock()
        if self is ThermostatsListRequestDeviceType.IGLOOHOME_LOCK:
            return igloohome_lock()
        if self is ThermostatsListRequestDeviceType.HUBITAT_LOCK:
            return hubitat_lock()
        if self is ThermostatsListRequestDeviceType.NOISEAWARE_ACTIVITY_ZONE:
            return noiseaware_activity_zone()
        if self is ThermostatsListRequestDeviceType.MINUT_SENSOR:
            return minut_sensor()
        if self is ThermostatsListRequestDeviceType.ECOBEE_THERMOSTAT:
            return ecobee_thermostat()
        if self is ThermostatsListRequestDeviceType.NEST_THERMOSTAT:
